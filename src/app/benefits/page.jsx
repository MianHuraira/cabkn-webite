    ...
    <div className={mounted ? "animate-fade-in" : "opacity-0"} style={{ minHeight: "100vh", background: "#fff" }}>
      ...
      >
        ...
        <div style={{ maxWidth: 1200, margin: "0 auto", padding: "0 24px", textAlign: "center", position: "relative", zIndex: 1 }}>
          <span style={{ color: "rgba(255,255,255,0.6)", fontSize: 14, fontWeight: 600, letterSpacing: 3, textTransform: "uppercase" }}>Why Ride With Us</span>
      ...
      <section style={{ marginTop: -40, position: "relative", zIndex: 2, padding: "0 24px" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto", display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 16 }}>
          {highlights.map((h, i) => (
            <div key={i} style={{ background: "#fff", borderRadius: 16, padding: "24px 20px", textAlign: "center", boxShadow: "0 8px 30px rgba(0,0,0,0.06)", border: "1px solid rgba(0,0,0,0.04)" }}>
              <h3 style={{ fontSize: 32, fontWeight: 700, color: h.color, margin: 0 }}>{h.value}</h3>
              <p style={{ fontSize: 13, color: "#6b7280", margin: "4px 0 0" }}>{h.label}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <SectionReveal>
        <div style={{ maxWidth: 1200, margin: "80px auto", padding: "0 24px" }}>
          <div style={{ textAlign: "center", marginBottom: 48 }}>
            <span style={{ color: "#004a70", fontSize: 13, fontWeight: 600, letterSpacing: 2, textTransform: "uppercase" }}>Everything You Need</span>
            <h2 style={{ fontSize: "clamp(1.5rem, 3vw, 2rem)", fontWeight: 700, color: "#1a1a2e", margin: "12px 0 0" }}>The Cabkn Advantage</h2>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: 20 }}>
            {benefits.map((b, i) => (
              ...
              >
                <div style={{ width: 48, height: 48, borderRadius: 12, background: "#eef2f6", display: "flex", alignItems: "center", justifyContent: "center", color: "#004a70", flexShrink: 0 }}>{b.icon}</div>
                <div>
                  <h3 style={{ fontSize: 16, fontWeight: 600, color: "#1a1a2e", margin: "0 0 6px" }}>{b.title}</h3>
      ...
      <SectionReveal>
        <div style={{ maxWidth: 800, margin: "80px auto", padding: "0 24px", textAlign: "center" }}>
          <h2 style={{ fontSize: "clamp(1.3rem, 2.5vw, 1.8rem)", fontWeight: 700, color: "#1a1a2e", margin: "0 0 12px" }}>Start Enjoying These Benefits Today</h2>
          <p style={{ color: "#6b7280", fontSize: 15, maxWidth: 500, margin: "0 auto 24px", lineHeight: 1.7 }}>Sign up now and experience the best way to travel between Nevis and Saint Kitts.</p>
